...
It is shown that snowfall and snow depth are the least important features for predicting air quality with purely ground-based measurements, as expected for the reason stated before. It can also be seen that meteorological conditions are generally more important than AOD measurements, which inspired training a new “slim” model on only the meteorology dataset and the RMSE of this new model is 0.263 and its REC curve and feature importance are shown below:  

![](assets/IMG/slim_3_4_REC.png)  

![](assets/IMG/slim_3_4_feature.png)  

These results show that during March and April of 2022, meteorological conditions are indeed the dominant factors for predicting air quality since the increase in model performance brought by the addition of AOD data is negligible.  
...
The RMSE for the full-feature decision tree model is 0.460 and 0.444 for the “slim” decision tree. Their REC curves and feature importances are shown below (top: full-feature, bottom: “slim” version with partial features):  
...
It can be seen that with more pollution from wildfire, the addition of actual particulate matter measurements in the form of AOD, even though they are sparse, can increase model performance significantly. In other words, when the particulate matter concentration level varies more due to more pollution, relying on only meteorology to predict air quality is insufficient, as expected.  
